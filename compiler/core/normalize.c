 ...
 *
 * $Header: /baseline/SNACC/compiler/core/normalize.c,v 1.11 2004/04/06 15:13:41 gronej Exp $
 * $Log: normalize.c,v $
...
TypeDef* AddConsObjectAssignment PROTO ((Module *m, ObjectAssignment *oa, Type *t, BasicType *bt, char *suffix));
    ...
    char *suffix)
{
     ...
     */
    newDef->definedName = Malloc (strlen (td->definedName) + strlen (suffix) + 4);
...
//    Tag  *lastTag;
     ...
     */
    newDef->definedName =
        Malloc (strlen (oa->objectName) +
               strlen (suffix) + 4);

    strcpy (newDef->definedName, oa->objectName);
    strcat (newDef->definedName, suffix);
    end = strlen (newDef->definedName);
    digit  = 1;
