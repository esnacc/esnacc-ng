...
void doubleDecodeTest(void)
{
  ...
  try
  {
     ...
     if (octs.BDecPdu(asnBuf, len))
     {
        if (octs.c_ustr()[0] == '1')
        {
           std::cout << "First OCTET STRING value: " << octs.c_ustr()[0] << std::endl;
           ...
           if (octs.BDecPdu(asnBuf, len))
           {
              ...
              {
                 std::cout << "Unexpected result for second OCTET STRING: "
                           << octs.c_ustr()[0] << std::endl;
              }
           }
           else
        ...
        {
           std::cout << "ERROR: Unexpected result for first OCTET STRING: "
                     << octs.c_ustr()[0] << std::endl;
        }
     }
     else
...
void octsTest(void)
{
  unsigned char buf[] = {0x04,0x14,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x30,0x41,0x42,0x43,0x44,0x45,0x46,0x47,0x48,0x49,0x50};
